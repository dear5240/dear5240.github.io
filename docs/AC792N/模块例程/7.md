...
代码跑SFC，数据在DDR, CPU时钟360M, DDR时钟250M,SFC时钟96M, flash 2线模式的coremark单核测试结果:

![img](img/95.png)

[超频测试,不可取] 跑DDR, CPU时钟420M, DDR时钟250M, Dhrystone单核测试结果:![](img/96.png)

代码跑SFC，数据在DDR, CPU时钟360M, DDR时钟250M,SFC时钟96M, flash 2线模式的coremark单核测试结果:![](img/97.png)

代码跑SFC，数据在SDRAM, CPU时钟360M, SDRAM时钟192M,SFC时钟96M, flash 2线模式的coremark双核测试结果:![](https://doc.zh-jieli.com/AC792/zh-cn/wifi_video_master/module_example/system/module_example/system/images/coremark_test0.png)

代码跑SFC，数据在DDR, CPU时钟360M, DDR时钟250M,SFC时钟96M, flash 2线模式的coremark双核测试结果:

![img](img/98.png)

代码跑SFC，数据在DDR, CPU时钟360M, DDR时钟250M,SFC时钟96M, flash 4线模式的coremark双核测试结果:

![img](img/99.png)

跑DDR, CPU时钟360M, DDR时钟250M,coremark双核测试结果:

![img](img/100.png)

[超频测试,不可取] 跑DDR, CPU时钟420M, DDR时钟250M, coremark双核测试结果:

![img](img/101.png)

## 7.3.4. 常见问题

* AC792N CoreMark/MHz 是单核2.13， 双核4.16的水平;
* AC792N DMIPS/MHz 是单核1.15的水平;
* 通常影响CPU性能测试结果的因素有:
  * 代码运行在内部ddr,sdram,flash会依次降低cpu性能;
...
本示例模拟了多种情况下程序异常的情况,以及异常分析与定位的方法， 更多详细软件调试相关方法
...
分别测试demo_hello跑sfc模式、demo_hello跑sdram模式的系统启动时间。

测试条件：`<span class="pre">SYS-360M</span>`、`<span class="pre">DDR-250M</span>`、`<span class="pre">HSB-180M</span>`、`<span class="pre">LSB-48M</span>`、`<span class="pre">SFC-96M</span>`、`<span class="pre">FLASH-4_5_0_0</span>`，`<span class="pre">关闭SDK和uboot打印</span>`。

## 7.29.1. demo_hello跑SDRAM的启动时间

1.demo_hello跑SDRAM，关闭LZ4代码压缩

![image1](img\131.png)

2.demo_hello跑SDRAM，开启LZ4代码压缩
...
1. 由于SDRAM的参数配置和SDRAM测试都在uboot中实现，当配置SDK为跑SDRAM时，uboot运行完成会需要更多时间。
2. 开启 `CONFIG_LZ4_COMPRESS_APP_CODE_ENABLE`宏时，在download阶段，下载工具会将app层的程序压缩后下载到Flash中。uboot则会在加载app层程序时将代码解压缩，因此在开启代码压缩条件下，uboot的运行时间会加长，并且代码量越大，时间越长。
3. AC792N上电到开始执行uboot需要26ms左右。
...
为了节省内存或者提升性能, 用户想把整个库例如 `orig_libx.a`文件的全部代码定位到内部ram或者flash或者sdram:
...
再在 `app_main.c` 中，分别定义：一个未初始化的全局数组、一个已初始化的全局数组、const常量一个字符串，并查看使用细节，即每个函数、变量的存放位置和大小。
...
- 具体请参考[库符号重定位到指定段方法](#731-库符号重定位到指定段方法)
...
① `isd_config_rule.c` 文件中 `SDRAM_PLL3_NOUSB_EN` 和 `SDRAM_PLL3_EN` 选项全配置0。 当系统时钟SYS_CLK>320M并且 < 384M时：SDRAM时钟为SYS_CLK*1.5/2，SYS_CLK >= 384M时：SDRAM时钟为SYS_CLK/2。 当系统时钟SYS_CLK>240M并且 < 320M时：SDRAM时钟为SYS_CLK*1.5/2；SDRAM时钟为SYS_CLK/2或SYS_CLK*1.5/2，此时 `SDRAM_CL=2` 更改无效。 系统时钟配置详情[系统各模块时钟介绍](#736-系统各模块时钟介绍)。
...
`low_power_all_group_force_sleep()` 休眠函数使用不能使用在开启蓝牙，可以使用在开启wifi，但开启wifi情况下需要注意掉线等问题（休眠时间不能过长，一般1秒内）

使用该函数是wifi的RF和操作系统都会进入休眠状态。
...
RF硬件休眠的禁止和允许函数，和强制休眠函数 `low_power_all_group_force_sleep()` 无关，当强制休眠则禁止RF硬件休眠函数也无效。

当需要禁止RF硬件自动休眠，则调用low_power_hw_unsleep_lock(); 当需要允许RF硬件自动休眠，则调用low_power_hw_unsleep_unlock();

当开启禁止RF硬件自动休眠，则操作系统不会进入休眠。
...
设置系统空闲休眠时间和操作系统相关，只有当操作系统都处于空闲状态才能进入休眠，休眠时间也根据操作系统空闲时间进行休眠，一般为20ms-50ms，用户需要加大休眠时间达到降低功耗，则使用power_set_sys_sleep_internal_us()函数自定义时间，

同时app_config.h的 `TCFG_LOWPOWER_LOWPOWER_SEL` 需要或上 `SYS_SLEEP_BY_IDLE` 和 `SYS_SLEEP_EN`。
...
1. 修改 `include_lib/driver/cpu/对应cpu目录/asm/cpu.h` 的宏定义 `#define CPU_CORE_NUM     1`
2. 在 `app_main.c` 的 `irq_info_table` 显式增加需要安装到核1的中断,其余没有显式增加的中断会全部安装到核0
3. 最后核0运行操作系统, 核1运行到cpu1_main,用户可以在此增加功能
...
**app_main.c 中断列表添加**：指定的中断号和优先级和指定核，**中断优先级务必设置为7才能生效**，如下为IRQ_TIMER4_IDX定时器4的中断号添加：

```c
//中断号    //指定中断优先级为7(最高为7)    //指定注册到核1(0/1)
#ifdef CONFIG_IPMASK_ENABLE  //不可屏蔽中断方法：支持写flash，但中断函数和调用函数和const要全部放在内部ram
...
## 7.40.3. API Reference

---

# 7.41. VDDIO_SYSVDD_DCDC14系统电压配置说明
...
具体档位在对应的cpu路径下的 `power_interface.h` 文件，例如AC790:VDDIOM_VOL_22V - VDDIOM_VOL_36V； AC791:VDDIOM_VOL_28V - VDDIOM_VOL_35V。
...
详情查看系统部分的[VDDIO_SYSVDD_DCDC14系统电压配置说明](#741-vddio_sysvdd_dcdc14系统电压配置说明)
...
在 `cpu\wl82\tools` 目录中提供了多个uboot文件选择，需要使用到单备份升级时必须选用支持单备份升级的uboot；debug支持指该uboot是否支持输出调试信息，当选择支持debug的uboot后，需要在串口中输出uboot的调试信息时，需要修改 `cpu\wl82\tools\isd_config_rule.c` 文件中UTTX参数。默认使用的uboot文件名为 `uboot.boot`，因此需要使用非默认uboot时，需要更名为 `uboot.boot`，或者修改 `cpu\wl82\tools\download.c` 中-uboot参数，更换为对应的uboot名称。uboot类型如下：
...
[USB做masstorge从机方法](https://doc.zh-jieli.com/AC792/zh-cn/wifi_video_master/module_example/peripherals/usb.html)

**情况3**：SD卡分为两个分区(隐藏SDFILE区，FAT1区)

**情况4**：SD卡分为三个分区(隐藏SDFILE区，隐藏FAT区，FAT1区)
