...
**1、播放GIF**
	已经封装好成api函数使用，路径 `apps/common/ui/gif_api.c`

**2、播放JPG**

    已经封装好成api测试线程，路径`apps/wifi_camera/ui_camera_demo/sd_play_jpg_mp3.c`
...
**4、有关图像合成应用**
    ...
    在camera出图时需要图像上显示一些时间信息或者大头框等![img](img/93.png)
...
*/
void*net_avi_play_start(constchar*url,avi_player_event_callback_tevent_callback,void*event_priv);
...
/*
    ...
    return：无
*/
voidnet_avi_play_stop(void*_net_player);
...
/*
    ...
    return：0 success；其他 fail
*/
intnet_avi_play_pause_resume(void*_net_player,intpause);
...
/*
    ...
    return：0 success；其他 fail
*/
intnet_avi_play_get_info(void*_net_player,structavi_info*info);
```
